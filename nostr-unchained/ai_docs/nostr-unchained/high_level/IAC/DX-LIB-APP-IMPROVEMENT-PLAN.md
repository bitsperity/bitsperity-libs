...
- [ ] [LIB] DX‑API fehlt: `hasExtension()` (DX_OPTI §1; CLEANUP B1)
- [ ] [LIB] DX‑API fehlt: `useExtensionSigner()` (DX_OPTI §1; CLEANUP B1)
- [ ] [LIB] DX‑API fehlt: `useLocalKeySigner()` (DX_OPTI §1; CLEANUP B1)
- [ ] [LIB] DX‑API fehlt: `getSigningInfo()` (DX_OPTI §1; CLEANUP B1)
- [ ] [LIB] Alias fehlt: `getStats()` → `getRelayStats()` (DX_OPTI §1; CLEANUP B1)
- [ ] [LIB] `publish` akzeptiert kein `string` (Overload `publish(content, kind=1)`) (DX_OPTI §4; CLEANUP B1)
- [ ] [LIB] Versions-Log immer aktiv (nicht an `debug` gebunden) (DX_OPTI §3; CLEANUP B2)
- [ ] [LIB] `getDM()` vor Signer `undefined` – Dokumentation/TS-Typisierung klarstellen (DX_OPTI §2; CLEANUP B3)
- [ ] [LIB] Docs: DM‑Ready‑Flow & Lazy Inbox erklären (DX_OPTI §2; CLEANUP A2)
- [ ] [LIB] Docs: Rumor‑ID vs. Wrap‑ID und Mapping beschreiben (CLEANUP A1)
- [ ] [LIB] Optional: unwrap‑Fehlerpfade besser debugbar machen (CLEANUP A1)
- [ ] [LIB] Raw-Key-Zweig in `_initializeCache()` entfernen (CLEANUP J1/J2/H/I)
...
- [ ] [APP] `NostrService`-Wrapper abbauen; neue Lib-APIs direkt nutzen (DX_OPTI §6)
- [ ] [APP] DM erst nach `initializeSigning()` verwenden (DX_OPTI §2)
- [ ] [APP] Keine doppelte Giftwrap-Subscription (Lazy via `.with()` oder explizit) (DX_OPTI §2)
- [ ] [APP] Publish via Fluent/Overload; kein `string`→`UnsignedEvent`-Fehler (DX_OPTI §4)
